...
export const legacyDefaultProjectPrompt =
  'Create high-converting mobile playable ad background variants similar to the reference image. Use English for all in-ad text and CTA by default, or use the requested language when the prompt says Language: <language>. Keep product intent, readable mobile composition, and a clean CTA-safe lower area. Do not draw hand cursors, scan boxes, CTA buttons, or tap/click cue text into the image; runtime overlays will add tap, drag, swipe, text cue, click, and CTA animation.';

export const defaultProjectPrompt =
  'Create high-converting mobile playable ad background variants similar to the reference image. Keep product intent, readable mobile composition, and fill the full 9:16 canvas without empty lower padding or white margins. Use the requested language only when the prompt explicitly asks for in-image text. Do not draw hand cursors, scan boxes, CTA buttons, tap/click cue text, or other editor UI into the image; runtime overlays can add interaction later.';
