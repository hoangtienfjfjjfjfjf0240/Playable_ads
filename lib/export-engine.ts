...
export function generateImagePlayableHtml({
  ...
  const frameBaseWidth = orientation === 'landscape' ? 640 : 360;
  const frameBaseHeight = orientation === 'landscape' ? 360 : 640;
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box;-webkit-tap-highlight-color:transparent}
    ...
    .scan-face{border:0;border-radius:12px;background:linear-gradient(#f97316,#f97316) left top/26% 3px no-repeat,linear-gradient(#f97316,#f97316) left top/3px 26% no-repeat,linear-gradient(#f97316,#f97316) right top/26% 3px no-repeat,linear-gradient(#f97316,#f97316) right top/3px 26% no-repeat,linear-gradient(#f97316,#f97316) left bottom/26% 3px no-repeat,linear-gradient(#f97316,#f97316) left bottom/3px 26% no-repeat,linear-gradient(#f97316,#f97316) right bottom/26% 3px no-repeat,linear-gradient(#f97316,#f97316) right bottom/3px 26% no-repeat;box-shadow:none;animation:psFaceScan var(--scan-speed) ease-in-out infinite}
    ...
    .ps-cta{position:absolute;left:var(--cta-x);top:var(--cta-y);z-index:8;width:var(--cta-width);max-width:88%;min-height:42px;padding:0 14px;border:0;border-radius:8px;background:linear-gradient(180deg,var(--cta-from),var(--cta-to));color:var(--cta-text);font-family:inherit;font-size:17px;font-weight:900;letter-spacing:0;text-align:center;text-shadow:0 1px 1px rgba(0,0,0,.22);box-shadow:inset 0 1px 0 rgba(255,255,255,.46),inset 0 -5px 0 rgba(0,0,0,.14),0 12px 24px rgba(var(--cta-shadow-rgb),.34);transform:translate(-50%,-50%);rotate:var(--cta-rotation);cursor:pointer;overflow:hidden;white-space:nowrap}
...
<body>
  ...
  (function(){
    ...
    var baseWidth = ${frameBaseWidth};
    var baseHeight = ${frameBaseHeight};
    function fitFrame(){
      if (!frame) return;
      var scale = Math.min(window.innerWidth / baseWidth, window.innerHeight / baseHeight);
      frame.style.setProperty("--ps-frame-scale", String(Math.max(0.1, scale)));
    }
    window.addEventListener("resize", fitFrame, { passive: true });
    window.addEventListener("orientationchange", fitFrame);
    if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", fitFrame, { once: true });
    else fitFrame();
  })();
  </script>
  <script>
...
.ps-cta-positioned{position:fixed!important;left:${clamp(layer.ctaX, 0, 100)}vw!important;top:${clamp(layer.ctaY, 0, 100)}vh!important;right:auto!important;bottom:auto!important;width:${clamp(layer.ctaWidth, 44, 92)}vw!important;max-width:88vw!important;min-height:42px!important;padding:0 14px!important;border:0!important;border-radius:8px!important;background:linear-gradient(180deg,${ctaColorFrom},${ctaColorTo})!important;color:${ctaTextColor}!important;font-size:17px!important;font-weight:900!important;text-align:center!important;text-shadow:0 1px 1px rgba(0,0,0,.22)!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.46),inset 0 -5px 0 rgba(0,0,0,.14),0 12px 24px rgba(${ctaShadowRgb},.34)!important;z-index:2147482999!important;transform:translate(-50%,-50%)!important;rotate:${clamp(layer.ctaRotation || 0, -180, 180)}deg!important;overflow:hidden!important;white-space:nowrap!important}
...
#ps-scan-effect.scan-face{border:0;border-radius:12px;background:linear-gradient(#f97316,#f97316) left top/26% 3px no-repeat,linear-gradient(#f97316,#f97316) left top/3px 26% no-repeat,linear-gradient(#f97316,#f97316) right top/26% 3px no-repeat,linear-gradient(#f97316,#f97316) right top/3px 26% no-repeat,linear-gradient(#f97316,#f97316) left bottom/26% 3px no-repeat,linear-gradient(#f97316,#f97316) left bottom/3px 26% no-repeat,linear-gradient(#f97316,#f97316) right bottom/26% 3px no-repeat,linear-gradient(#f97316,#f97316) right bottom/3px 26% no-repeat;box-shadow:none;animation:psScanFace ${clamp(layer.scanSpeed, 400, 5000)}ms ease-in-out infinite}
...
#ps-hand-click{position:fixed;left:${clamp(layer.handX, 0, 100)}vw;top:${clamp(layer.handY, 0, 100)}vh;width:${clamp(layer.handSize, 32, 260)}px;z-index:2147483000;pointer-events:none;filter:drop-shadow(0 9px 14px rgba(0,0,0,.32));rotate:${clamp(layer.handRotation || 0, -180, 180)}deg;animation:psHand${animName(layer.handMotion)} 1.1s infinite ease-in-out}
</style>
