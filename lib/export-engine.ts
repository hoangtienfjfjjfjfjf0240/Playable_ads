...
export const legacyDefaultProjectPrompt =
  'Create high-converting mobile playable ad background variants similar to the reference image. Remove all in-image text by default, including headlines, labels, and CTA copy from the reference. Only keep a short primary headline when the prompt explicitly asks for in-image text. Keep product intent, readable mobile composition, and a clean CTA-safe lower area. Keep important content away from the bottom safe zone instead of pushing it too low. Do not leave empty text bars, blank cards, placeholder buttons, or hollow UI shells after removing copy. Do not draw hand cursors, scan boxes, CTA buttons, or tap/click cue text into the image; runtime overlays will add tap, drag, swipe, text cue, click, and CTA animation.';

export const defaultProjectPrompt =
  'Create high-converting mobile playable ad background variants similar to the reference image. Keep product intent, readable mobile composition, and fill the full 9:16 canvas without empty lower padding or white margins. Remove all in-image text from generated variants by default, including text already visible in the reference image. Only keep one short primary headline when the prompt explicitly asks for in-image text. Respect the bottom safe zone: do not place important content too close to the lower edge, and keep the lower area simple enough for runtime CTA overlays without turning it into a blank strip. When text or UI chrome is removed from the reference, do not leave behind empty bars, blank search fields, placeholder cards, or hollow button shapes; rebuild those areas with natural background detail. Do not draw hand cursors, scan boxes, CTA buttons, tap/click cue text, or other editor UI into the image; runtime overlays can add interaction later.';
